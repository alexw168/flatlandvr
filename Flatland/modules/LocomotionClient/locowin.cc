...
static GLfloat global_velocity_scale = 5.0; /* velocity scaling */
GLfloat worldvx = 0.0;           /* calculated values for velocities */
GLfloat worldvy = 0.0;
GLfloat worldvz = 0.0;
GLfloat worldvyaw = 0.0;

static float mapping1( float in, float outmax ) 
{
...
static float mapping2( float in, float outmid ) 
{
	...
	if (button == GLUT_LEFT_BUTTON) 
	{
		...
		if (state == GLUT_DOWN) 
		{
			...
			if(slider_index>=0) 
			{
				...
				if ( LocomotionClient )
				{
					LOCOMOTION_CLIENT_MEMBERS *LocomotionClientMembers = (LOCOMOTION_CLIENT_MEMBERS *) LocomotionClient->ptr1;
					LocomotionClientMembers->VelocityScale = global_velocity_scale;
					LocomotionClientMembers->Velocity.x = worldvx;
					LocomotionClientMembers->Velocity.y = worldvy;
					LocomotionClientMembers->Velocity.z = worldvz;
					LocomotionClientMembers->Velocity.yaw = worldvyaw;
				}
			}
